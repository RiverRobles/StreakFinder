...
def load_existing_labels(path: str, dataset_name: str, username: str) -> pd.DataFrame:
    ...
    if len(existing_users) == 0:
        ...
        for i, value in enumerate(col_values):
            df = pd.concat([df, pd.DataFrame({'index': [i + 1], 'label':[value], 'username': [username]})], ignore_index=True)

    return df 

def upsert_label(path: str, sample_index: int, label: int, notes: str = "", user_id: str = "", username: str = "", dataset_name: str="") -> None:
    os.makedirs(os.path.dirname(path) or '.', exist_ok=True)
    df = load_existing_labels(path, dataset_name, username)
    ...
    ws.update_cell(sample_index + 1, ind, label)
...
with col_decision_left:
    if shortcut_button("Not Streaked ❌", "arrowleft", key="not_streaked_left"):
        upsert_label(st.session_state.labels_path, current_index, 0, "", user_id, username, dataset_name)
        st.session_state.index = min(num_samples - 1, current_index + 1)
        st.rerun()
...
with col_decision_right:
    if shortcut_button("Streaked ✅", "arrowright", key="streaked_right"):
        upsert_label(st.session_state.labels_path, current_index, 1, "", user_id, username, dataset_name)
        st.session_state.index = min(num_samples - 1, current_index + 1)
        st.rerun()
