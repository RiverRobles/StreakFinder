...
# --------- App ---------
st.set_page_config(page_title="Streak Labeler", page_icon="✨", layout="wide")
st.title("Streak Labeler - Sinusoid Presence Annotation")
...
# If current index already labeled, move to first unlabeled (unless explicitly jumped)
if not apply_jump and st.session_state.index in done_indices and len(done_indices) < num_samples:
...
with col_info:
    ...
    with c2:
        if st.button("Skip ➡️"):
            ...
            st.rerun()
    with c3:
        #if st.button("Not Streaked ❌"):
        if shortcut_button("Not Streaked ❌", "arrowleft"):
            upsert_label(st.session_state.labels_path, current_index, 0, notes)
            st.session_state.index = min(num_samples - 1, current_index + 1)
            st.rerun()
    with c4:
        #if st.button("Streaked ✅"):
        if shortcut_button("Streaked ✅", "arrowright"):
            upsert_label(st.session_state.labels_path, current_index, 1, notes)
            st.session_state.index = min(num_samples - 1, current_index + 1)
            st.rerun()
